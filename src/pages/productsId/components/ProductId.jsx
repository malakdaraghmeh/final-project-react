...
export default function ProductsId() {
  ...
  const handelSubmit = async (e) => {
    ...
    setLoader(true);
    try {
      const { data } = await axios.post(`/products/${id}/review`, values, {
  ...
  return (
    <div className="padding-bottom">
      ...
      <form
        className="container align-items-center d-flex justify-content-md-center"
      ...
      <h2 className="text-center pb-2 ">Reviews</h2>
      <div className="container w-50 d-flex flex-column justify-content-center gap-4 bg-light ">
      {product.reviews.map((revi) => (
          <div
            className="review d-flex flex-column  bg-white align-items-center"
            key={revi._id}
          >   
          <div className="d-flex align-items-center gap-2 nameq">
          <img className="usrimage" src={revi.createdBy.image.secure_url}/>
          <h2 className="revww">{revi.createdBy.userName}</h2>
          </div>
          <div >
               <span className="revww">{revi.updatedAt }</span>
              <h2 className="revww">{revi.comment}</h2>
          </div>
